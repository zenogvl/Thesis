...
#Read data 
dataIn <- read.csv("Data/FromOriginalPapers/Kroencke_2023_raw.csv")



#Get the emotions from the colnames and by removing part of the string.  
emotions <- gsub("int_", "", colnames(dataIn)[164:175])
...
dataComb <- matrix(NA, nrow = nrow(dataIn), ncol = 15, dimnames = list(NULL, c("ID", "Time", "Wave", emotions))) %>%
  as.data.frame()
...
#Write clean data
write.csv(data, "Data/Clean/Kroencke_2023.csv")
